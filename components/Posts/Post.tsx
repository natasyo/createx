...
import { GetPostsQuery } from "@/generatesTypes/gql/graphql";
import Image from "next/image";
import NoImage from "@/public/no-image.jpg";
import Link from "next/link";
type Props = {
  post: NonNullable<GetPostsQuery["posts"]>["edges"][number]["node"];
  className: string;
};

function Post({ post, className }: Props) {
  return (
    <div className={`${className} rounded`}>
      <div className={`lg:h-[306px] h-[250px]  lg:mb-6 mb-3`}>
        {post.featuredImage ? (
          <Image
            ...
            className={`max-h-full object-cover min-w-full h-full w-full`}
          />
        ) : (
          <Image
            src={NoImage.src}
      ...
      <div className={`lg:p-6 p-4`}>
        <p className={`font-bold text-xl mb-3`}>{post.title}</p>
        <p className={`mt-3 mb-4 flex items-center text-sm text-gray-700`}>
          ...
          {post.commentCount || ` No comments`}
        </p>

        <div dangerouslySetInnerHTML={{ __html: post.excerpt || "" }} />
        <Link href={post.uri!}>Читать далее</Link>
      </div>
    </div>
  );
}
