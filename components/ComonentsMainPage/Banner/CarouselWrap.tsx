// @flow
"use client";
import * as React from "react";
import { ReactNode, useEffect, useRef, useState } from "react";
import { Carousel, IconButton } from "@material-tailwind/react";

type Props = {
  children: ReactNode;
  className?: string;
};
export const CarouselWrap = ({ children, className }: Props) => {
  const ref = useRef<HTMLDivElement>(null);
  const [navigationWidth, setNavigationWidth] = useState(
    ref.current && ref.current.offsetWidth,
  );
  useEffect(() => {
    setNavigationWidth(ref.current && ref.current.offsetWidth);
  }, [navigationWidth]);

  return (
    <Carousel
      navigation={({ setActiveIndex, activeIndex, length }) => (
        <div className={` w-full mx-auto absolute lg:bottom-30 bottom-16`}>
          <div className={`container mx-auto`}>
            <div
              className={`z-50 flex gap-2 max-w-full pl-3 w-[45.25rem] ${className}`}
              ref={ref}
            >
              {new Array(length).fill("").map((_, i) => (
                <span
                  key={i}
                  className={`block cursor-pointer transition-all max-w-full border-b border-solid border-gray-800 text-2.5xl font-bold text-gray-800 font-ubuntu ${
                    i === activeIndex
                      ? "border-opacity-100"
                      : "border-opacity-30 text-opacity-60"
                  }`}
                  style={{
                    width: `${navigationWidth && navigationWidth / length}px`,
                  }}
                  onClick={() => setActiveIndex(i)}
                >
                  {i < 9 ? `0${i + 1}` : i + 1}
                </span>
              ))}
            </div>
          </div>
        </div>
      )}
      prevArrow={({ handlePrev }) => (
        <IconButton
          onClick={handlePrev}
          className={`!absolute top-2/4 left-4 bg-white bg-opacity-50 shadow-none hover:shadow-none rounded-full hover:bg-opacity-100`}
        >
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M9.20711 17.2071C8.81658 17.5976 8.18342 17.5976 7.79289 17.2071L3.29289 12.7071C2.90237 12.3166 2.90237 11.6834 3.29289 11.2929L7.79289 6.79289C8.18342 6.40237 8.81658 6.40237 9.20711 6.79289C9.59763 7.18342 9.59763 7.81658 9.20711 8.20711L6.41421 11L20 11C20.5523 11 21 11.4477 21 12C21 12.5523 20.5523 13 20 13L6.41421 13L9.20711 15.7929C9.59763 16.1834 9.59763 16.8166 9.20711 17.2071Z"
              fill="#424551"
            />
          </svg>
        </IconButton>
      )}
      nextArrow={({ handleNext }) => (
        <IconButton
          onClick={handleNext}
          className={`!absolute top-2/4 right-4 bg-white bg-opacity-50 shadow-none hover:shadow-none rounded-full hover:bg-opacity-100`}
        >
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M14.7929 6.79289C15.1834 6.40237 15.8166 6.40237 16.2071 6.79289L20.7071 11.2929C21.0976 11.6834 21.0976 12.3166 20.7071 12.7071L16.2071 17.2071C15.8166 17.5976 15.1834 17.5976 14.7929 17.2071C14.4024 16.8166 14.4024 16.1834 14.7929 15.7929L17.5858 13H4C3.44772 13 3 12.5523 3 12C3 11.4477 3.44772 11 4 11H17.5858L14.7929 8.20711C14.4024 7.81658 14.4024 7.18342 14.7929 6.79289Z"
              fill="#424551"
            />
          </svg>
        </IconButton>
      )}
    >
      {children}
    </Carousel>
  );
};
